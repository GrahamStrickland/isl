...
\begin{itemize}
  \item[(a)] We have an R plot of various error curves in
    Figure~\ref{fig2_3err}.

    \begin{figure}[!ht]
      \includegraphics[scale=0.6, center]{../plots/ex2_3_a.pdf}
      \caption{Typical bias, variance, training error, test error,
        and Bayes error
      \label{fig2_3err}}
    \end{figure}
...
dev.off()
  ...
  \item[(b)] We are given that the training error will always be greater than
    ...
    be less than the (squared) bias, while the test error will
    decrease as the model
    becomes more flexible. The Bayes error is irreducible, so it
    remains constant.
\end{itemize}
