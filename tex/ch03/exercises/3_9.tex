...
\begin{itemize}
  \item[(a)] In Figure~\ref{fig3_9scat}, we have a scatterplot matrix of all the
    ...
    \end{figure}
  \item[(b)] We have the following output from using the
    \verb|cor()| function:
...
F-statistic: 252.4 on 7 and 384 DF,  p-value: < 2.2e-16
    ...
    \end{itemize}
  \item[(d)] In Figure~\ref{fig3_9diag}, we have some dianostic plots
    of the linear
    ...
    \begin{figure}[!ht]
      ...
      \caption{Diagnostic plots for regression of \texttt{Auto} data
      \label{fig3_9diag}}
    \end{figure}
    We can see that the residuals vs fitted plot shows a well-defined
    shape to the
    data that may suggest a non-linear relationship. We can see some
    high-leverage
  ...
  \item[(e)] Yes, we can see the interaction between
    \verb|horsepower| and \verb|year|
...
F-statis$tic: 392.5 on 3 and 388 DF,  p-value: < 2.2e-16
        ...
        \end{verbatim}\normalsize
    All $p$-values are less than $2 \times 10^{-16}$ and we have
    $R^2$ statistics
    both around 3/4 compared with the values around 2/3 for linear regression
    of \verb|mpg| on \verb|horsepower| and \verb|age| without an
    interaction term,
