    ...
    \begin{proof}
      ...
      \begin{equation*}
        \begin{split}
          ...
          \Biggl[\sum_{i=1}^n {(y_i - x_i\hat{\beta})}^2\Biggr] \\
          = &\, \Biggl(\sum_{i'=1}^n x_{i'}^2\Biggr)
          \Biggl(\sum_{i=1}^n y_i^2 - \sum_{i=1}^n
          {(x_i\hat{\beta})}^2\Biggr) \\
          = &\, \Biggl(\sum_{i'=1}^n x_{i'}^2\Biggr)
...
F-statistic: 344.3 on 1 and 98 DF,  p-value: < 2.2e-16
        \end{verbatim}\normalsize
    We can see that the $t$-statistic given by 18.556 for the
    regression of \verb|y|
    onto \verb|x| is approximately the same as the $t$-statistic
    given by 18.56 of
    \verb|x| onto \verb|y|.
\end{itemize}
